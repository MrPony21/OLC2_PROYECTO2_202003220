    ...
    def ejecutar(self, out, env):
        ...
        if valor_sym.type == Type.STRING:
            ...
            try:
                ...
                valor_parseado = math.floor(valor_parseado_float)
                return Symbol(self.line, self.column, valor_parseado, Type.INTEGER)
            except ValueError:
                x = ("Error: Cadena incompatible para convertirse a integer")
                out.addErrores(x, env.name, self.line, self.column, "Semantico")
                return Symbol(self.line, self.column, None, Type.NULL)


        else:
            x = ("Error: no se puede parsear un ",valor_sym.type," a integer")
            out.addErrores(x, env.name, self.line, self.column, "Semantico")
            return Symbol(self.line, self.column, None, Type.NULL)
